        ...
        server.createContext("/kasus/api/tambah", new HttpHandler() {
            ...
            public void handle(HttpExchange exchange) throws IOException {
                if ("POST".equals(exchange.getRequestMethod())) {
                    ...
                    String kronologis_kasus = userData.get("kronologis_kasus");
                   
                    


                    // Tambahkan user ke database
                    new APIKasus().addKasusToDatabase(no_registrasi, no_identitas, user_id, nama, jenis_kasus, layanan, kronologis_kasus);
...
//                    int userId = Integer.parseInt(pathParts[4]); // Ambil ID dari URLSystem.out.println("ok");
                    if ("POST".equals(exchange.getRequestMethod())) {
                        ...
                        String layanan = userData.get("layanan");
                        String kronologis_kasus = userData.get("kronologis_kasus");
                        new APIKasus().updateKasusInDatabase(userId, no_registrasi, no_identitas, user_id, nama, jenis_kasus, layanan, kronologis_kasus);
