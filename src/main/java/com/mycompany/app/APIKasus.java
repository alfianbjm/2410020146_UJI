...
public class APIKasus implements HttpHandler {
    ...
    private void handleAddKasus(HttpExchange exchange) throws IOException {
        try (InputStream inputStream = exchange.getRequestBody();
            ...
            String kronologis_kasus = requestBody.get("kronologis_kasus");

            addKasusToDatabase(no_registrasi, no_identitas, user_id, nama, jenis_kasus, layanan, kronologis_kasus);
    ...
    private List<Map<String, Object>> getKasusFromDatabase() {
        ...
        String sql = "SELECT kasus.id as id, kasus.no_registrasi as no_registrasi, pemohon.no_identintas, " +
             "users.username, kasus.nama, kasus.jenis_kasus, kasus.layanan, kasus.kronologis_kasus " +
             "FROM kasus " +
        ...
        try (Connection conn = DB.getConnection();
            ...
            while (rs.next()) {
                ...
                data.put("kronologis_kasus", rs.getString("kronologis_kasus"));
                datas.add(data);
            }
    ...
    private void handleEditKasus(HttpExchange exchange, String path) throws IOException {
        ...
        try {
            ...
            try (InputStream inputStream = exchange.getRequestBody();
                ...
                updateKasusInDatabase(id, no_registrasi, no_identitas, user_id, nama, jenis_kasus, layanan, kronologis_kasus);
    ...
    void addKasusToDatabase(String no_registrasi, String no_identitas,String user_id,String nama, String jenis_kasus, String layanan, String kronologis_kasus) {
        String sql = "INSERT INTO kasus (no_registrasi, no_identitas, user_id, nama, jenis_kasus, layanan, kronologis_kasus) VALUES (?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = DB.getConnection();
            ...
            pstmt.setString(7, kronologis_kasus);
            pstmt.executeUpdate();
    ...
    void updateKasusInDatabase(int id, String no_registrasi, String no_identitas, String user_id, String nama, String jenis_kasus, String layanan, String kronologis_kasus) {
        
        String sql = "UPDATE kasus SET no_registrasi = ?, no_identitas = ?, user_id = ?, nama = ?, jenis_kasus = ?, layanan = ?, kronologis_kasus = ? WHERE kasus.id = ?";
        try (Connection conn = DB.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
